...
def apply_category_view(request):
    customer = models.Customer.objects.get(user_id=request.user.id)
    category = CMODEL.Category.objects.all()
    return render(request, 'customer/apply_category.html', {'category': category, 'customer': customer})
...
def payment(request):
    ...
    return render(request, 'customer/payment.html', {'policies': policies, 'customer': customer})
...
def compare_policy(request, category):
    customer = models.Customer.objects.get(user_id=request.user.id)
    category = CMODEL.Category.objects.all().filter(category_name=category)
    cat = category[0]
    policy = CMODEL.Policy.objects.all().filter(category=cat.id)
    context = {'policies': policy, 'category': cat.category_name, 'customer':customer}
    return render(request, 'customer/compare_policies.html', context)


def policy_list(request):
    category = CMODEL.Category.objects.all()
    context = {'categories': category}
    return render(request, 'customer/policy_list.html', context)
